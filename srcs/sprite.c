...
void 	draw_sprite(t_cub *cub, t_sprite *s)
{
	...
	while (++x < WIDTH && x < s->screen_x + s->screen_width/2)
	{
		...
		y = HEIGHT/2 - s->screen_height/2 -1;
		while (++y < HEIGHT && y < HEIGHT/2 + s->screen_height/2)
		{
			true_y = (y - (HEIGHT/2 - s->screen_height/2)) * s->sprite_img.height / s->screen_height; 
			if (y < 0)
				continue ;
...
void	calculate_sprite_info(t_cub *cub, t_sprite *sprite)
{
	...
	//sprite->screen_x = (WIDTH/2) * (1 + sprite->plane_proj/sprite->dir_proj);
	sprite->screen_height = HEIGHT / sprite->norm;
	sprite->screen_width = HEIGHT / sprite->norm;
