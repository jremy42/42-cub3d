...
void	calculate_sprite_info(t_cub *cub, t_sprite *sprite)
{
	...
	sprite->dir_proj = vector_det(sprite->cam_pos_x, sprite->cam_pos_y, cub->player.dir_x, cub->player.dir_y) / cub->player.dir_norm;
}
...
int		print_sprite_info(t_sprite *sprite)
{
	...
	printf("norm/plane_proj/dir_proj : [%f/%f/%f]\n", sprite->norm, sprite->plane_proj, sprite->dir_proj);
	return (1);
}
